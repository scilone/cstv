...
final readonly class TmdbMediaMetadataProvider implements MediaMetadataProvider
{
    ...
    public function match(string $kind, string $title, ?int $year, string $locale): ?array
    {
        ...
        $items = $this->items($response, $locale);
        return $items[0] ?? null;
    }
    ...
    private function items(array $response, string $locale): array
    {
        $results = $response['results'] ?? [];
        if (!is_array($results)) return [];
        return array_values(array_filter(array_map(fn (mixed $row): ?array => $this->item($row, $locale), $results)));
    }
    /** @return array<string, mixed>|null */
    private function item(mixed $row, string $locale): ?array
    {
        ...
        return ['id' => $kind . ':' . (int) $row['id'], 'kind' => $kind, 'title' => $title, 'originalTitle' => is_string($row['original_title'] ?? $row['original_name'] ?? null) ? ($row['original_title'] ?? $row['original_name']) : null, 'releaseYear' => $year, 'overview' => is_string($row['overview'] ?? null) ? $row['overview'] : null, 'rating' => is_numeric($row['vote_average'] ?? null) ? round((float) $row['vote_average'], 1) : null, 'posterUrl' => $image($row['poster_path'] ?? null, 'w500'), 'backdropUrl' => $image($row['backdrop_path'] ?? null, 'w780'), 'ageRatingFr' => $this->ageRatingFr($kind, (int) $row['id'], $locale)];
    }
