...
* **Sauvegarde CSTV optionnelle (F36)** : Après validation par le panel, les identifiants restent toujours mémorisés sur l'appareil. Lorsqu'un compte CSTV est lié, la case « Sauvegarder mes identifiants IPTV dans CSTV » permet, sur consentement explicite, d'en conserver une unique copie chiffrée au niveau du compte CSTV pour la restaurer silencieusement sur un nouvel appareil. La case est décochée par défaut ; la décocher, se déconnecter du compte CSTV ou se déconnecter d'IPTV révoque la copie distante selon le parcours concerné.
...
* **Fiche film Android TV refondue (F28)** : Présentation cinéma pleine page remplaçant la transposition de la fiche mobile. L'affiche occupe la moitié gauche de l'écran, plein bord, fondue vers le fond à droite ; la colonne de droite empile le titre en grand, la ligne de métadonnées (année, genres, durée, note), le synopsis tronqué à six lignes, les étiquettes cliquables réalisateur/acteurs, une rangée d'actions icône + libellé séparées par des filets (favoris, j'aime, je n'aime pas), puis le bouton de lecture en pilule pleine largeur sans icône (dédoublé en « Reprendre la lecture » / « Relire depuis le début » si une position de reprise existe). Le focus arrive directement sur le bouton de lecture. La rangée « Titres associés » dépasse volontairement en bas d'écran et remonte de la distance exacte manquante, calculée à partir des hauteurs réellement mesurées, quand le focus y descend. L'accentuation jaune hors charte a disparu au profit d'`AccentLavande`. Fiche mobile et fiche série inchangées.
  * **Ajustements PO (F30)** : le synopsis est pondéré et ne peut donc plus chasser les boutons de lecture hors de l'écran ; s'il est tronqué, une action « VOIR PLUS » ouvre le texte intégral dans une fenêtre plein écran plutôt que de déplier en place. Les boutons de lecture reprennent le dessin mobile (rayon 8 dp, texte blanc) sur toute la largeur de la colonne. Le trailer joue dans l'emplacement de l'affiche et non plus en fond de page. La rangée « Titres associés » adopte le sélecteur fixe commun aux écrans TV (`tvPivotEnabled` opt-in sur `RelatedTitlesRow`, mobile et série inchangés) et se tient à 48 dp des bords.
...
## 25. Favori en appui long sur les cartes de chaînes Android TV (F25)
Cette fonctionnalité assainit les interactions au D-Pad de l'onglet Live TV en supprimant la cible de focus secondaire portée par l'étoile de favori, au profit d'un appui long sur la carte entière.
...
* **Exclusion de la rangée « Récemment regardées » (F25-R2)** : Cette rangée reste volontairement en dehors du contrat d'appui long pour éviter tout conflit avec l'action préexistante de retrait d'historique (qui utilise déjà l'appui long avec confirmation pour le retrait).
* **Aperçu vidéo miniature retiré** : la prévisualisation vidéo en direct au survol d'une carte, livrée initialement avec F25, a été retirée : le rendu de la vidéo dans la miniature n'était pas satisfaisant et le démarrage du décodeur pesait sur la fluidité de la navigation au D-pad.

---

## 26. Ajustement de la hauteur et des interlignes EPG sur les cartes Live TV (F26)
Cette évolution résout les problèmes de troncature verticale des informations de grille des programmes (EPG) sur les cartes de chaînes Live TV (`StreamTvCard`).
...
* **Favori en appui long propagé à la grille de catégorie** : la grille verticale d'affichage de catégorie spécifique (`LazyVerticalGrid`) partage la même carte `StreamTvCard`, donc le même geste de favori en appui long qu'en mode « Tout ».
...
* **Autorisation permanente par contenu (évolution v1.90.0)** : L'écran de saisie du PIN propose une case « toujours autoriser ce contenu sur ce profil ». Cochée, elle transforme le déverrouillage ponctuel en autorisation durable pour ce profil et cette œuvre entière (film, ou série — jamais un épisode isolé), synchronisée dans le cloud comme le niveau de restriction. Aucun écran ne permet encore de la révoquer en V1.
