...
- `RelatedTitlesSelector` (`domain/model/`, objet pur générique) : tri par **nombre de genres communs décroissant** (3 puis 2 puis 1), départage par score `0.5*note + 0.3*même catégorie + 0.2*fraîcheur d'ajout` (même `categoryId` que le média courant, récupéré via `getStreamById` ; année de sortie non stockée en base → écartée) ; exclut les candidats sans genre commun ; limité à 10.
