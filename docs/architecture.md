...
* **Prévisualisation vidéo au survol de chaîne et favori en appui long sur Android TV (F25)** :
  - **Gestion d'état de prévisualisation (`LiveChannelPreview.kt`)** : Centralisation de la logique temporelle stable de 1000 ms, de l'activation, de l'annulation et de la gestion d'erreurs au sein d'un composant de support dédié `LiveChannelPreview.kt`. L'état réactif est représenté par `LiveChannelPreviewState`.
  - **Single Shared Player Lifecycle** : Utilisation d'un unique lecteur `ExoPlayer` d'aperçu instancié au niveau de la mise en page de l'écran (`TvLayout`) par `rememberLiveChannelPreviewState`. L'état est stable et n'est pas recréé lors des changements d'état hors ligne ou d'identifiants (répercutés en place via `SideEffect`), éliminant ainsi les fuites de lecture en tâche de fond (F25-R1). Le cycle de vie est relié au `LifecycleEventObserver` pour couper immédiatement la lecture en cas de mise en arrière-plan (`ON_STOP`).
  - **Bannissement temporaire des flux en échec** : Les chaînes dont le chargement ou la préparation du flux a échoué (via le callback `onPlayerError`) sont enregistrées dans un ensemble d'échecs (`failedStreams`) et exclues de toute nouvelle tentative de lecture d'aperçu pour la session active.
...
* **Ajustement de la hauteur et des interlignes EPG sur les cartes Live TV (F26)** :
  ...
  - **Propagation sans duplication de décodeurs** : Raccordement de l'unique état de prévisualisation `previewState` à la grille de catégorie (`LazyVerticalGrid`), propageant le comportement d'aperçu et de favori en appui long de manière asymétrique et optimisée sans instancier aucun second lecteur ou décodeur.
