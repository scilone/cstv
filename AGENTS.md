...
- Explicitement hors périmètre, jamais ajouter sans demande explicite PO : catch-up/timeshift, multi-comptes Xtream (plusieurs identifiants distincts), enregistrement (PVR), autre protocole IPTV.
- **Restriction d'âge par profil (contrôle parental) : dans le périmètre depuis F44** (livrée 2026-08-19), sur demande explicite PO — écart assumé à l'exclusion par défaut. Un profil peut être bridé sur une classification française (Tous publics/10/12/16/18) ; la lecture d'une œuvre au-dessus du niveau autorisé (ou non classifiée) est refusée. Le déverrouillage ponctuel et la modification du niveau exigent un PIN à 4 chiffres, propre à l'appareil, jamais synchronisé dans le cloud (le niveau d'âge du profil, lui, l'est). Voir `ai/features/archive/F44-restriction-age-par-profil.md` pour le détail complet (règles métier, architecture, écarts V1 assumés). **Évolution (v1.90.0, 2026-08-20)** : la saisie du PIN pour débloquer une lecture propose une case « toujours autoriser ce contenu sur ce profil » — l'autorisation posée (œuvre entière, film ou série) est mémorisée par profil et synchronisée dans le cloud comme le niveau d'âge (le PIN reste local à l'appareil). Aucun écran de révocation en V1.
...
- Base actuelle : `AppDatabase`, version **29** (voir `app/src/main/java/.../data/local/db/AppDatabase.kt`). Les migrations 25 → 26 ajoutent `ProfileEntity.remoteId` (index unique), 26 → 27 créent `ProfileSyncStateEntity`, 27 → 28 normalise les identités cloud, et 28 → 29 ajoute les titres nettoyés, attributs et clés de liaison du catalogue.
