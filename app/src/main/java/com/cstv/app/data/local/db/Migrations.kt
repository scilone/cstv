...
/**
 * F44 : niveau d'âge autorisé du profil (contrôle parental). Colonne nullable
 * ajoutée sans backfill : tous les profils existants restent non bridés
 * (`null`), conformément à la décision « non bridé par défaut ».
 */
val MIGRATION_35_36 = object : Migration(35, 36) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE profiles ADD COLUMN maxAgeRating INTEGER")
    }
}

/** F44 hotfix : classification T22 persistée par média, y compris l'absence de classification. */
val MIGRATION_36_37 = object : Migration(36, 37) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("CREATE TABLE IF NOT EXISTS content_classifications (kind TEXT NOT NULL, providerId INTEGER NOT NULL, title TEXT NOT NULL, year INTEGER, ageRating INTEGER, resolvedAt INTEGER NOT NULL, PRIMARY KEY(kind, providerId))")
    }
}

/**
 * F45 (évolution F44) : autorisation permanente d'un média pour un profil
 * bridé, accordée lors d'un déverrouillage PIN. Table neuve, aucun backfill —
 * aucune autorisation permanente n'existait avant cette version.
 */
val MIGRATION_37_38 = object : Migration(37, 38) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS parental_media_authorizations (" +
                "profileId INTEGER NOT NULL, mediaUid INTEGER NOT NULL, grantedAt INTEGER NOT NULL, " +
                "PRIMARY KEY(profileId, mediaUid), " +
                "FOREIGN KEY(profileId) REFERENCES profiles(id) ON DELETE CASCADE, " +
                "FOREIGN KEY(mediaUid) REFERENCES media_refs(mediaUid) ON DELETE CASCADE)"
        )
        db.execSQL("CREATE INDEX IF NOT EXISTS index_parental_media_authorizations_mediaUid ON parental_media_authorizations(mediaUid)")
    }
}
