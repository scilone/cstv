...
import com.cstv.app.data.remote.dto.CatalogVideoDto
...
class TrailerRepositoryImplTest {
    ...
    private val repository = TrailerRepositoryImpl(
        xtream, catalogApiService, credentialsManager, XtreamRequestGate(), trailerCacheDao, timeProvider, CoroutineScope(Dispatchers.Unconfined)
    )
    ...
    @Test
    fun fallbackUsesCstvCatalogVideos() = runBlocking {
        whenever(catalogApiService.videos("movie:10")).thenReturn(CatalogVideosResponseDto(listOf(CatalogVideoDto("YouTube", "dQw4w9WgXcQ", "Trailer", true))))
    ...
    fun getTrailerPreview_usesFreshPersistentEntryWithoutNetworkCall() {
        runBlocking {
            ...
            whenever(trailerCacheDao.get("movie", 7)).thenReturn(
                TrailerCacheEntity("movie", 7, "dQw4w9WgXcQ", "youtube", null, 999L)
            )
    ...
    // L'Accueil demande un média avec son identifiant canonique, la fiche de
    // détails sans. Le cache est indexé sur (type, id catalogue) : le preview
    // rendu doit porter le média demandé, sinon la fiche rejette le résultat de
    // l'Accueil (`preview.media == trailerMedia` faux) et n'affiche aucun trailer.
    @Test
    ...
    fun getTrailerPreview_prefersXtreamAndCachesResult() {
        runBlocking {
            ...
            verify(xtream, times(1)).getVodInfo("user", "password", 7)
            verify(catalogApiService, times(0)).videos(org.mockito.kotlin.any(), org.mockito.kotlin.any())
            Unit
        }
    }

    @Test
    fun getTrailerPreview_fallsBackToCatalogForMovieAndSeries() {
        runBlocking {
            doReturn(credentials).whenever(credentialsManager).getCredentials()
            ...
            val videos = CatalogVideosResponseDto(listOf(CatalogVideoDto("YouTube", "dQw4w9WgXcQ", "Trailer", true)))
            whenever(catalogApiService.videos("movie:10")).thenReturn(videos)
            whenever(catalogApiService.videos("series:20")).thenReturn(videos)
    ...
    fun getTrailerPreview_rejectsInvalidSourceAndReturnsNullOnNetworkError() {
        runBlocking {
            ...
            whenever(catalogApiService.videos("movie:43")).thenThrow(IllegalStateException("offline"))
    ...
    fun clearSessionCache_forcesFreshResolutionIncludingNegativeEntries() {
        runBlocking {
            ...
            whenever(catalogApiService.videos("movie:44")).thenReturn(CatalogVideosResponseDto(emptyList()))
